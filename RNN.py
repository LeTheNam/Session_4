...
class RNN:
    ...
    def LSTM_layer(self, embeddings):
        ...
        initial_state = tf.contrib.rnn.LSTMStateTuple(zero_state, zero_state) # Khoi tao trang thai C va h 
        lstm_inputs = tf.unstack(tf.transpose(embeddings, perm= [1, 0, 2]))   # Embedding la mang 3 chieu vi ma tran dau vao 
                                                                              # la cac doan van ban gom nhieu tu
...
#                     rnn._final_tokens: train_final_tokens
            ...
            if train_data_reader._batch_id == 0:
                # Sau khi train xong 1 epoch thi tien hanh danh gia test data
